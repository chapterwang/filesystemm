...
const static string ACCESS[] = { "只读","可修改","可执行" };
const static string rootPath = "D:/VitualDisk/";
...
DiskMannger::DiskMannger()
{
	...
	//设置根节点的父节点为自身
	root->father = root;
	cout << "welcome input 'help' for help" << endl<< "\n[root@localhost "+rootPath+"]# ";
	string opear,cmd;
	while (cin >> cmd) 
	{
		if (cmd == "format") {
			...
			cout << "\n format:ge shi hua.\n"<<
				"●mkdir:chuang jian zi mu lu\n" <<
				"●rmdir : shan chu zi mu lu\n" <<
				"●ls : xian shi mu lu\n" <<
				"●cd : gen gai dang qian mu lu\n" <<
				"●create : chuang jian wen jian\n" <<
				"●open : da kai wen jian\n" <<
				"●close : guan bi wen jian\n" <<
				"●write : xie wen jian\n" <<
				"●read : du wen jian\n" <<
				"●rm : shan chu wen jian\n" <<
				"●exit : tui chu xi tong\n"
				<<endl;
		}
		else {
